...
nomes_clusters = {0: 'Grupo 4: Pesos-Pesados/Lendários', 1: 'Grupo 1: Estágios Iniciais', 2: 'Grupo 3: Tanques Defensivos', 3: 'Grupo 2: Atacantes Rápidos'}
df['Perfil'] = df['Cluster'].map(nomes_clusters)
sns.set_theme(style='whitegrid')
plt.rcParams.update({'font.size': 12})
plt.figure(figsize=(10, 6))
sns.scatterplot(data=df, x='speed', y='attack', hue='Perfil', palette='deep', alpha=0.7, s=60)
...
plt.close()
plt.figure(figsize=(10, 6))
sns.scatterplot(data=df, x='defense', y='hp', hue='Perfil', palette='deep', alpha=0.7, s=60)
...
plt.ylabel('')
for p in ax.patches:
    ax.annotate(f'{int(p.get_width())}', (p.get_width() + 5, p.get_y() + p.get_height() / 2.0), ha='left', va='center', fontsize=12)
plt.tight_layout()
